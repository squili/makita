...
impl Display for CustomIdType {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            Self::Debug => "WhyDoTheyCallItAnOven",
...
impl CustomIdType {
    ...
    pub fn from_str(from: &str) -> Result<Self> {
        match from {
            "WhyDoTheyCallItAnOven" => Ok(Self::Debug),
...
pub fn parse_custom_id(custom_id: &str) -> Result<(CustomIdType, HashMap<String, String>)> {
    ...
    for item in parts.iter().skip(1) {
        let parts = match item.split_once("=") {
            Some(parts) => parts,
            None => return Err(Error::new(BotError::InvalidRequest(format!("Invalid CustomID argument {}", item)))),
        };
        let lhs = parts.0;
        let rhs = parts.1;
