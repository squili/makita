// Copyright 2021 Squili
// This program is distributed under the terms of the GNU Affero General Public License
// You should have received a copy of the license along with this program
// If not, see <https://www.gnu.org/licenses/#AGPL>

use anyhow::{Result, Error};
use crate::error::BotError;
use std::collections::HashMap;

pub enum CustomIdType {
    ListPermissions,
}

impl CustomIdType {
    pub fn to_string(&self) -> String {
        match self {
            CustomIdType::ListPermissions => "ListPermissions",
        }.to_string()
    }

    pub fn from_str(from: &str) -> Result<Self> {
        match from {
            "ListPermissions" => Ok(Self::ListPermissions),
            _ => Err(Error::new(BotError::InvalidRequest(format!("Invalid CustomID type {}", from))))
        }
    }
}

pub fn parse_custom_id(custom_id: &String) -> Result<(CustomIdType, HashMap<String, String>)> {
    let parts = custom_id.split(";").skip(1).collect::<Vec<&str>>();
    let mut map = HashMap::with_capacity(parts.len()-1);
    for item in parts.iter().skip(1) {
        let parts = item.splitn(2, "=").collect::<Vec<&str>>();
        if parts.len() != 2 {
            return Err(Error::new(BotError::InvalidRequest(format!("Invalid CustomID argument {}", item))));
        }
        let lhs = unsafe { parts.get_unchecked(0) };
        let rhs = unsafe { parts.get_unchecked(1) };
        map.insert(lhs.to_string(), rhs.to_string());
    }
    Ok((CustomIdType::from_str(parts.get(0).unwrap())?, map))
}

// make sure the custom id can never be over 100 characters!
pub fn build_custom_id(ty: &CustomIdType, map: &Option<HashMap<String, String>>) -> String {
    let mut buf = String::new();
    buf.extend("MAK;".chars());
    buf.extend(ty.to_string().chars());

    if let Some(map) = map {
        for (key, value) in map {
            buf.extend(format!(";{}={}", key, value).chars());
        }
    }

    buf
}
