...
impl<'r, T> Decode<'r, Postgres> for SqlId<T> where T: From<u64> + Into<u64> {
    fn decode(value: <Postgres as HasValueRef<'r>>::ValueRef) -> Result<Self, BoxDynError> {
        Ok(SqlId(T::from(<i64 as Decode<Postgres>>::decode(value)? as u64)))
    }
}

impl<'r, T> Encode<'r, Postgres> for SqlId<T> where T: From<u64> + Into<u64> + Copy {
    fn encode(self, buf: &mut <Postgres as HasArguments<'r>>::ArgumentBuffer) -> IsNull where Self: Sized {
...
impl<T> Type<Postgres> for SqlId<T> where T: From<u64> + Into<u64> {
